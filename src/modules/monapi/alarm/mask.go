...
func SyncMaskconf() error {
	...
	acache.MaskCache.SetAll(maskMap)
...
func IsMaskEvent(event *models.Event) bool {
	...
	for i := 0; i < len(detail); i++ {
		...
		var exists bool
		if event.Category == 1 {
			maskTagsList, exists = acache.MaskCache.GetByKey("#" + event.Endpoint)
			if !exists {
				maskTagsList, exists = acache.MaskCache.GetByKey(eventMetric + "#" + event.Endpoint)
				if !exists {
					continue
				}
			}
		} else {
			maskTagsList, exists = acache.MaskCache.GetByKey(eventMetric + "#" + event.CurNid)
			if !exists {
				continue
			}
		}
