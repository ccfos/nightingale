...
type PrometheusRule struct {
	URLs                    []string `label:"URLs" json:"urls" description:"An array of urls to scrape metrics from"`
...
func (p *PrometheusRule) Validate() error {
	if len(p.URLs) == 0 && len(p.KubernetesServices) == 0 {
		return fmt.Errorf(" prometheus.rule unable to get urls or KubernetesServices")
	}
	return nil
}
