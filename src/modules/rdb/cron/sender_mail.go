...
func ConsumeMail() {
	...
	for {
		list := redisc.Pop(1, config.MAIL_QUEUE_NAME)
		if len(list) == 0 {
...
func sendMail(message *dataobj.Message) {
	...
	switch config.Config.Sender["mail"].Way {
	case "api":
	...
	default:
		logger.Errorf("not support %s to send mail, mail: %+v", config.Config.Sender["mail"].Way, message)
	}
}

func sendMailByAPI(message *dataobj.Message) {
	api := config.Config.Sender["mail"].API
