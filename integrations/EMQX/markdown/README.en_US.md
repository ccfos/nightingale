The emqx plugin collects metrics through the emqx dashboard API.

1. Configuration and notes

```toml
# # Collect interval; the interval specified here overrides the interval in config.toml
# interval = 60
[[instances]]
## Address of the emqx dashboard, ip:port. For monitoring a single cluster, separate multiple IPs with commas; one available IP will be picked at random for the connection request
## For multiple clusters, split them into multiple instance configurations
addrees="http://192.168.11.11:18083,http://192.168.11.12:18083"
## Specify the emqx version. Supports v3/v4/v5, corresponding to emqx 3.X/4.X/5.X respectively
version="v3"
## The API requires authentication; provide the username and password
username="admin"
password="public"
```

2. The plugin first calls `/api/${version}/nodes` to get information such as the node version, running status, and connection count. The API returns:
```json
{
  "code": 0,
  "data": [
...
```
The corresponding metrics generated are as follows:
```
emqx_node_connections{node="xxx"}  10
emqx_node_status{node="xxxx"} 1 # 1 means running
...
emqx_node_uptime_seconds{node="xxxx"} 1234567 # in seconds
```
Two additional metrics are generated, indicating how many nodes in the current cluster are running and how many have stopped. The cluster label is the address from the configuration:
```
emqx_cluster_node_running{cluster="xxx"} 2
emqx_cluster_node_stopped{cluster="xxx"} 0
```

The plugin calls the `/api/${version}/stats` endpoint to get statistics for each node. The API returns:
```json
{
  "code": 0,
  "data": [
...
```
Based on the returned data, the following metrics are generated:
```
...
The plugin calls `/api/${version}/nodes/${node}/metrics` to get metrics:
```json
{
  "code": 0,
  "data": {
...
```
The corresponding metrics generated:
```
...
3. Miscellaneous
   emqx 5.X provides a prometheus endpoint. You can use the input.prometheus plugin configured with `http://ip:18083/api/v5/prometheus/stats` to collect metrics, with no extra username/password authentication required.
