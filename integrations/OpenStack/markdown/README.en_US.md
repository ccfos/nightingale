...
* Metrics(Metrics for OpenStack instances
...
Currently, this plugin requires the following APIs:
...
Since this plugin generates a large number of unique tags, it is **strongly recommended** to use modifiers such as `tagexclude` to drop unwanted tags in order to keep cardinality low.

For deployments with only a small number of VMs and hosts, a short polling interval (e.g. seconds or minutes) is acceptable. For large deployments, polling a large number of systems will impact performance. Use the `interval` option to change how often the plugin runs.

`interval`: how often to collect metrics. Setting this at the plugin level overrides the global agent interval setting.
...
* openstack_metrics
    ...
    * host
    * zone_name
    * disk_device_allocation
    * disk_device_capacity
    * disk_device_usage
    * disk_device_read_requests
    * disk_device_read_bytes
    * disk_device_write_requests
    * disk_device_write_bytes
    * disk_device_read_latency
    * disk_device_write_latency
    * network_outgoing_bytes
    * network_incoming_bytes
    * network_outgoing_packets
    * network_incoming_packets
    * network_incoming_packets_error
    * network_outgoing_packets_error
    * network_incoming_packets_drop
    * network_outgoing_packets_drop
    * volume_size
    * disk_root_size
    * disk_ephemeral_size
    * memory
...
Different services require different permissions; configure them according to your actual environment.

The [metrics] service only needs minimal permissions, so the `reader` role is sufficient. Through the `metrics` service you can collect CPU, memory, disk, network and other metrics of cloud instances.

The remaining services require the `admin` role. Through the `hypervisor` service you can collect information about the host machines, such as CPU, memory, and disk metrics.

If you need to collect metrics for both cloud instances and host machines at the same time, you can configure multiple `[[instances]]` blocks, each with different permissions.
...
[[instances]]
  ...
  ## User authentication credentials. Must have admin rights.
  username = "reader" # only collect cloud instance metrics, so the reader role is enough
  password = "password"
  ...
  ## "nova_services", "ports", "projects", "servers", "services", "stacks", "storage_pools",
  ## "metrics", "subnets", "volumes"
  enabled_services = ["metrics"] # only collect cloud instance metrics
...
[[instances]]
  ...
  ## User authentication credentials. Must have admin rights.
  username = "admin" # only collect host machine metrics, so the admin role is required
  password = "password"
  ...
  ## "nova_services", "ports", "projects", "servers", "services", "stacks", "storage_pools",
  ## "metrics", "subnets", "volumes"
  enabled_services = ["hypervisors"] # only collect host machine metrics
