...
由于这个插件生成了大量独特的标签，为了
保持基数较小，**强烈建议**使用修饰符类似于`tagexclude`丢弃不需要的标签。

对于只有少量虚拟机和主机的部署，较小的轮询间隔（如秒、分钟）是可以接受的。对于大型部署，轮询大量系统会影响性能。使用`interval`选项可以更改插件运行的频率。

`interval`：收集指标的频率。在插件级别设置此值将覆盖全局代理间隔设置。
...
不同service的权限不同，需要根据实陵情况进行配置。

[metrics]服务仅需要最小权限就能实现，可以配置`reader`权限。可以通过`metrics`服务获取云主机的cpu、内存、磁盘、网络等指标。

其余相关服务需要`admin`权限。可以通过`hypervisor`服务获取宿主机相关信息，如cpu、内存、磁盘等指标。

如需同时获取云主机和宿主机相关指标，可以配置多个`[[instances]]`，分别配置不同的权限。
...
[[instances]]
  ...
  ## User authentication credentials. Must have admin rights.
  username = "reader" # 仅获取云主机相关指标，配置reader权限
  password = "password"
  ...
  ## "nova_services", "ports", "projects", "servers", "services", "stacks", "storage_pools",
  ## "metrics", "subnets", "volumes"
  enabled_services = ["metrics"] # 仅获取云主机相关指标
...
[[instances]]
  ...
  ## User authentication credentials. Must have admin rights.
  username = "admin" # 仅获取宿主机相关指标，配置admin权限
  password = "password"
  ...
  ## "nova_services", "ports", "projects", "servers", "services", "stacks", "storage_pools",
  ## "metrics", "subnets", "volumes"
  enabled_services = ["hypervisors"] # 仅获取宿主机相关指标
