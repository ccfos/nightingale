...
可推荐的能力（仅从中挑选 / suggest only from these）：创建告警规则·订阅·屏蔽·通知规则·仪表盘；查询监控资源与配置；查询告警事件；告警根因排查；主机健康诊断与接入排障；数据源连通诊断与数据查询；生成 PromQL / SQL / 日志查询；通知消息模板与媒介通道配置；自愈脚本生成；categraf 部署安装指导；夜莺文档问答。
