...
**Jira params** (all values are strings; list/map values are JSON strings): `project_key`, `issue_type` (required); `on_resolve` = `close` (default: comment and transition to Done) / `comment` / `none`; `resolve_transition` (override the automatically picked Done transition); `on_repeat` = `none` (default) / `comment`; `reopen_transition` + `reopen_duration` (minutes, default 1440); `wont_fix_resolution`; `priority_map` e.g. `"{\"1\":\"Highest\",\"2\":\"High\"}"`; `labels` e.g. `"[\"n9e\"]"`; `tags_as_labels` = `"true"`; `fields` e.g. `"{\"customfield_10010\":\"{\\\"value\\\":\\\"prod\\\"}\"}"`. The Jira template has two fields: `title` (issue summary) and `content` (description, also used as the recovery comment).
