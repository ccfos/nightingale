...
| `request_type=slackwebhook` / `request_type=mattermostwebhook` | `{"webhook_url": "https://hooks.slack.com/services/...", "bot_name": "#ops-alerts"}` | The incoming webhook URL of the target channel |
...
| `jira` Jira issue (`request_type=jira`) | `project_key` + `issue_type` (dropdowns in the UI via `/api/n9e/jira-project-list/:id` and `/api/n9e/jira-issue-type-list/:id?project=`) | `jira/` |

**Jira params** (all values are strings; list/map values are JSON strings): `project_key`, `issue_type` (required); `on_resolve` = `close` (default: comment and transition to Done) / `comment` / `none`; `resolve_transition` (override the automatically picked Done transition); `on_repeat` = `none` (default) / `comment` (closed issues are never reopened: the same alert firing after the issue is closed creates a new one); `priority_map` e.g. `"{\"1\":\"Highest\",\"2\":\"High\"}"`; `labels` e.g. `"[\"n9e\"]"`; `tags_as_labels` = `"true"`; `fields` e.g. `"{\"customfield_10010\":\"{\\\"value\\\":\\\"prod\\\"}\"}"`. The Jira template has two fields: `title` (issue summary) and `content` (description, also used as the recovery comment).
