...
**新版（notify_version=1）校验会清空全部旧版字段**：`user_group_ids`、`redefine_channels`/`new_channels`、`redefine_webhooks`/`webhooks`、`redefine_severity`/`new_severity`（`models/alert_subscribe.go:Verify`）。也就是说**改写级别/渠道/回调是旧版能力**；新版想变级别/渠道，去通知规则层面做路由（→ n9e-notify-rule-copilot）。
