...
description: 夜莺(n9e)告警订阅规则(alert_subscribe)的创建、编辑、排障一站式助手。当用户要求"创建订阅规则 / 订阅告警 / 告警事件转发 / 抄送另一个团队 / 告警升级（持续 N 分钟未处理再通知谁）/ 跨业务组收告警 / 订阅不生效排查"时使用。订阅是通知阶段对事件的"复制+二次路由"；要配"事件直接通知给谁"用通知规则（→ notify-rule-copilot），要不收告警用屏蔽（→ alert-mute-copilot）。
tags:
  - internal
---
