...
| 告警规则 | `alert_rule` 表 | `models/alert_rule.go` | 否（用 `create-alert-rule`） |
| 通知模板 | `notify_tpl` 表 | `models/notify_tpl.go` | 否（用 `generate-message-template`） |
| 通知通道 | `notify_channel` 表 | `models/notify_channel.go` | 否（用 `notify-channel-copilot`） |
