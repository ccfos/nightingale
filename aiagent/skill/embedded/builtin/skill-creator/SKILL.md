...
2. **Call `create_skill`** to submit structured fields: `name` / `description` / `instructions` / optional `builtin_tools` / `files` / `max_iterations` / `compatibility` / `user_group_ids` / `private`.
   ...
   - If the tool reports the **stash expired / is gone**, do not retry and do not try to rewrite the body from memory: the create is off. Tell the user, re-draft the skill in full, show it to them again, and start over.
   - If the tool says it **couldn't stash the draft** (no conversation context — e.g. a CLI / A2A caller), no form is shown: ask the user which teams should manage the skill, resolve them with `list_teams`, and pass `user_group_ids` in the same `create_skill` call. That path never interrupts, so nothing is lost.
