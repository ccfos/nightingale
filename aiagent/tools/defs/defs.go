...
返回每条规则的 name / severity / prom_ql / for_duration / labels / annotations，不需要任何权限或业务组参数。`,
	Type: aiagent.ToolTypeBuiltin,
	Parameters: []aiagent.ToolParameter{
		{Name: "payload", Type: "string", Description: "Prometheus rule YAML 文本（与 prometheus 官方格式一致，支持顶层 groups、纯 rules 数组、单条 rule）", Required: true},
	},
}
...
返回每条规则的写入结果（id / error）。建议先调 preview_prom_rule_yaml 让用户确认。`,
	...
	Parameters: []aiagent.ToolParameter{
		...
		{Name: "payload", Type: "string", Description: "Prometheus rule YAML 文本（同 preview_prom_rule_yaml 的 payload）", Required: true},
...
var HTTPFetch = aiagent.AgentTool{
	Name: "http_fetch",
	Description: `GET 一个公网 URL，返回响应正文文本。用于把外部公开资源（如 GitHub raw 上的 Prometheus rule YAML、Grafana dashboard JSON）拉进当前会话。
