...
type BuiltinMetric struct {
	...
	Collector  string `json:"collector"`            // Type of collector (e.g., 'categraf', 'telegraf')
	Type       string `json:"type"`                 // Type of metric (e.g., 'host', 'mysql', 'redis')
...
func (bm *BuiltinMetric) Verify() error {
	...
	bm.Type = strings.TrimSpace(bm.Type)
	if bm.Type == "" {
...
func AddBuiltinMetrics(ctx *ctx.Context, metrics []BuiltinMetric) error {
	...
	for i := range metrics {
		...
		if err := metrics[i].Verify(); err != nil {
			...
			// should we continue to process the next item and delete? for example metrics = append(metrics[:i], metrics[i+1:]...)
			// or just return the error?
			// or log the error and continue?
			return err
		}
	}
	return BatchInsert(ctx, metrics)
}
...
func BuiltinMetricGets(ctx *ctx.Context, createdBy, collector interface{}) ([]BuiltinMetric, error) {
	var lst []BuiltinMetric
	// TODO: query condition is unclear, need to be improved
	err := DB(ctx).Where("created_by = ? or collector = ? ", createdBy, collector).Find(&lst).Error
	if err != nil {
		return nil, err
	}
