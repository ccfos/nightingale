...
type BuiltinMetric struct {
	...
	Collector      string          `json:"collector" gorm:"size:191;not null;index:idx_collector,sort:asc;comment:'type of collector'"`
	Typ            string          `json:"typ" gorm:"size:191;not null;index:idx_typ,sort:asc;comment:'type of metric'"`
	Name           string          `json:"name" gorm:"size:191;not null;index:idx_builtinmetric_name,sort:asc;comment:'name of metric'"`
	Unit           string          `json:"unit" gorm:"size:191;not null;comment:'unit of metric'"`
	Note           string          `json:"note" gorm:"size:4096;not null;comment:'description of metric'"`
	Lang           string          `json:"lang" gorm:"size:191;not null;default:'zh';index:idx_lang,sort:asc;comment:'language'"`
	Translation    []Translation   `json:"translation" gorm:"type:text;serializer:json;comment:'translation of metric'"`
	Expression     string          `json:"expression" gorm:"size:4096;not null;comment:'expression of metric'"`
	ExpressionType string          `json:"expression_type" gorm:"size:32;not null;default:'promql';comment:'expression type: metric_name or promql'"`
	MetricType     string          `json:"metric_type" gorm:"size:191;not null;default:'';comment:'metric type like counter/gauge'"`
