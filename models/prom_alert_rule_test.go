...
func TestConvertAlert(t *testing.T) {
	...
	convJobMissing := models.ConvertAlert(jobMissing[0], "30s", "1", 0)
...
`), &ruleEvaluationSlow)
	...
	convRuleEvaluationSlow := models.ConvertAlert(ruleEvaluationSlow[0], "1m", "1", 0)
...
`), &targetMissing)
	...
	convTargetMissing := models.ConvertAlert(targetMissing[0], "1h", "1", 0)
