...
前端对 AI 回复进行结构化解析：
- 尝试从 Markdown 内容中提取 JSON 格式的查询语句（PromQL / SQL）
- 如果提取成功，渲染为带语法高亮的 QueryResultBlock，附带「运行」和「复制」按钮
- 否则直接按 Markdown 渲染
...
| 字段 | 类型 | 说明 |
|------|------|------|
| page | string | 页面类型，可选值：`dashboards` / `alert_history` / `active_alert` / `explorer` |
| param | AssistantPageInfoParam | 页面参数 |

### AssistantPageInfoParam
...
| 字段 | 类型 | 说明 |
|------|------|------|
| content_type | string | 内容类型：`markdown` / `hint` |
| content | string | 回复内容（处理完成后填充） |
| stream_id | string | 流式 ID，用于订阅 SSE 流 |
...
```json
{
  "dat": {
    ...
    "response": [
      {
        "content_type": "markdown",
        "content": "你可以使用以下 PromQL 查询 CPU 使用率：\n\n```promql\n100 - (avg by(instance) (rate(node_cpu_seconds_total{mode=\"idle\"}[5m])) * 100)\n```",
        "stream_id": "b2c3d4e5-f6a7-8901-bcde-f12345678901",
        "is_finish": true,
        "is_from_ai": true
      }
...
2. 检查 `is_finish` 字段
3. 为 `true` 时停止轮询，渲染最终 `response[0].content`
