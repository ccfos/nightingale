...
type AlertRuleTestFireForm struct {
	...
	SkipSend  bool             `json:"skip_send"`  // true 时全链路干跑，不实际发送通知
...
// 同一 用户+业务组+规则 的触发间隔下限，防止误连点骚扰真实接收人
const testFireMinInterval = 10 * time.Second

var testFireLastRun sync.Map
...
func (rt *Router) alertRuleTestFire(c *gin.Context) {
	...
	limitKey := fmt.Sprintf("%d:%d:%d", me.Id, bgid, f.Config.Id)
	if last, ok := testFireLastRun.Load(limitKey); ok && time.Since(last.(time.Time)) < testFireMinInterval {
		ginx.Bomb(http.StatusTooManyRequests, "test fire too frequently, please retry later")
	}
	testFireLastRun.Store(limitKey, time.Now())
	...
	pipelineStage, processedEvent, eventDropped := rt.runTestFirePipelines(cfg, event, username)
	...
	if eventDropped {
		...
		// ---- Stage 5: 通知匹配与发送 ----
		stages = append(stages, rt.runTestFireNotify(cfg, event, f.SkipSend))
...
func (rt *Router) runTestFirePipelines(cfg *models.AlertRule, event *models.AlertCurEvent, username string) (testFireStage, *models.AlertCurEvent, bool) {
	pipelineIds := make([]int64, 0)
	for _, pc := range cfg.PipelineConfigs {
		if pc.Enable && pc.PipelineId > 0 {
			pipelineIds = append(pipelineIds, pc.PipelineId)
		}
	}
	...
	pipelines, err := models.GetEventPipelinesByIds(rt.Ctx, pipelineIds)
	if err != nil {
		return testFireStage{Stage: "pipeline", Status: testFireStageFail, Data: gin.H{"error": err.Error()}}, nil, false
	}
	...
	status := testFireStagePass
	results := make([]gin.H, 0, len(pipelines))
	current := event
	dropped := false
	for _, p := range pipelines {
		item := gin.H{"id": p.ID, "name": p.Name}
		resultEvent, result, err := workflowEngine.Execute(p, current, triggerCtx)
		if err != nil {
			item["status"] = "failed"
		...
		item["node_results"] = result.NodeResults
		if resultEvent == nil {
			item["dropped"] = true
			dropped = true
	...
	return testFireStage{Stage: "pipeline", Status: status, Data: gin.H{"pipelines": results}}, current, dropped
}
...
func (rt *Router) runTestFireNotify(cfg *models.AlertRule, event *models.AlertCurEvent, skipSend bool) testFireStage {
	...
	for _, nrid := range cfg.NotifyRuleIds {
		...
		for i := range nr.NotifyConfigs {
			...
			if err := dispatch.NotifyRuleMatchCheck(&nc, event); err != nil {
				item["matched"] = false
			...
			resp, err := SendNotifyChannelMessage(rt.Ctx, rt.UserCache, rt.UserGroupCache, nc, []*models.AlertCurEvent{event})
			if err != nil {
				item["sent"] = false
				item["error"] = err.Error()
				anyFail = true
