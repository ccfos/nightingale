...
func Initialize(configDir string, cryptoKey string) (func(), error) {
	...
	builtinComponentCache := memsto.NewBuiltinComponentCache(ctx, syncStats, config.Center.BuiltinIntegrationsDir)
	...
	centerRouter := centerrt.New(config.HTTP, config.Center, config.Alert, config.Ibex,
		...
		redis, sso, ctx, metas, idents, targetCache, userCache, userGroupCache, userTokenCache, builtinComponentCache)
